...
/**
 * プレイヤーが獲得できるマスの一覧を返す。 盤面が空（初手）なら必ず中央パネル（13番）のみ。 反転できるマスがあれば必ずそのマスのみ。無ければ点灯済みパネルに隣接する空きマス。
 *
 * @param board 現在の盤面
 * @param playerId 配置するプレイヤーのID
 ...
 */
const isValidPanelIndex = (index: number | null): index is number =>
  index !== null && Number.isInteger(index) && index >= 0 && index < PANEL_COUNT;
 ...
 */
export const computeAttack25Board = (logs: SeriarizedGameLog[], attackChanceEnabled: boolean) => {
  let board: Attack25Board = Array.from({ length: PANEL_COUNT }, () => null);
  let attackChanceUsed = false;
  for (const log of logs) {
    if (log.actionType !== "correct" || !isValidPanelIndex(log.panel) || !log.playerId) continue;
    const panel = log.panel;
    const removedPanel = log.removedPanel;
    // 既に埋まっているパネルは破損ログとみなし、ログ畳み込みを止めずに無視する
    ...
    board = applyReversiFlip(board, panel, log.playerId);
    if (isValidPanelIndex(removedPanel) && board[removedPanel] !== null) {
      // 消去対象も範囲内かつ点灯済みのときだけ反映する
      board[removedPanel] = null;
    }
