...
適切な粒度でコミットを行ってください。コミットメッセージは`prefix: message`の形式で、1行以内に簡潔に記述してください。
...
- コメントは原則として記載しないでください。命名やコードで意図が伝わるようにし、JSDocやインラインコメントは真に必要な場合のみ最小限にとどめてください。
- 関数の返り値やAPIの返り値は必ず`as const`を使用してください。
...
- **本番の設定**: Workerランタイムのシークレット（`TURSO_*` / `BETTER_AUTH_SECRET` / `GOOGLE_*` / `STRIPE_*`）は `wrangler secret put` で登録済み。ビルド時にクライアントへ埋め込む `VITE_APP_URL` は Workers Builds のビルド変数に設定します。Workers Builds のビルドイメージはNodeを`.node-version`から読みますが（`package.json`は読みません）、pnpmはイメージ同梱の古い版が使われるため、ビルド変数`PNPM_VERSION`に`packageManager`と同じバージョン（現在は`12.4.1`）を設定します
